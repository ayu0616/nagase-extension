...
const pushScoreButtons = () => {
    const buttons = [...document.querySelectorAll("#paper-field > div:last-child > table > tbody > tr:not(.jss182) > td:nth-child(10) > div > button:not(:disabled)")];
...
const createPushButton = () => {
    const pushButtonElem = createElementFromHTML("<button class='renda-button'>点数入力欄を連打</button>");
...
const autoSubmitScore = () => {
    const scoreInputElems = [...document.querySelectorAll("#adornment-score-0")];
    scoreInputElems.forEach((elem) => (elem.autocomplete = "off"));
    const dialogContainers = [...document.querySelectorAll("div[role='dialog']")];
    ...
    dialogContainers.forEach(async (dialogContainer, i) => {
        if (scoreInputElems[i].value) {
            const submitButton = dialogContainer.querySelector('button[data-test="submit-button"]');
            submitButton.click();
            await sleep(100);
...
const createHideButton = () => {
    ...
    const hideButtonElem = createElementFromHTML("<button class='hide-button'>" + buttonTexts[0] + "</button>");
    hideButtonElem.addEventListener("click", () => {
        isDisplayNone = !isDisplayNone;
        window.addEventListener("click", () => {
            const tr = [...document.querySelectorAll("div#paper-field tr")];
...
const getTableData = () => {
    ...
    const datas = [...trList].map((tr) => {
        const texts = tr.querySelector("td:nth-child(6)")?.innerHTML.split(" ");
        const yearMatch = texts[1].match(/[0-9]{4}/);
        return {
            elem: tr,
            univName: texts[0],
            year: yearMatch[0],
...
const queryTr = (as_id) => {
    ...
    const tr = trList.find((tr) => tr.children[3].innerHTML === as_id);
    return tr;
};
/**fetchで答案のデータを取得 */
const fetchData = async () => {
    ...
    const data = await fetch("https://production-apprunner-api.toshin-correction.com/sheets/assigned", {
        headers: headers,
    }).then((res) => res.json());
...
const getCurrentValues = () => {
    const selectByUnivElem = document.getElementById("by-univ-name");
    const selectByYearElem = document.getElementById("by-year");
    return {
...
const isPairExists = (datas, univName, year) => {
...
const setSearchOptions = () => {
    ...
    const selectByUnivElem = document.getElementById("by-univ-name");
    const univOptions = [...selectByUnivElem.children];
    ...
    const selectByYearElem = document.getElementById("by-year");
    const yearOptions = [...selectByYearElem.children];
...
const fillScore = () => {
    ...
    dialogContainers.forEach((container) => {
        ...
        if (!(pdfObjectElem && inputElem))
            return; // 両方が揃っていないときは何もしない
